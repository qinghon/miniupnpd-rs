...
fn GetListOfPortMappings(h: &mut upnphttp, action: &str, ns: &str) {
	...
	body.push_str(format!("<u:{}Response xmlns:u=\"{}\"><NewPortListing><![CDATA[", action, ns).as_str());
	...
	body.push_str(format!("]]></NewPortListing></u:{action}Response>").as_str());
	...
	if let Some(p) = GetValueFromNameValueList(&data, "NewDefaultConnectionService") {
		if cfg!(feature = "strict") {
			...
			if !p.starts_with("uuid:00000000-0000-0000-0000-000000000000") {
				...
				BuildSendAndCloseSoapResp(
					h,
					format!("<u:{action}Response xmlns:u=\"{ns}\"></u:{action}Response>").as_bytes(),
				);
			}
		} else {
			info!("{}({}) : Ignored", action, p);
...
pub fn ExecuteSoapAction(h: &mut upnphttp) {
	...
	// todo: move to stack
	let action = h.get_req_str_from(h.req_soapActionOff).to_string();
